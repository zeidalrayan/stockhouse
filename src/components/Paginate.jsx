...
export default function DataTable({ allBarang }) {
  const columns = columnsData.map((col) => ({
    ...
    renderCell: (params) => {
      ...
      if (columnKey === "harga") {
        return formatrupiah(getKeyValue(item, columnKey));
      } else if (columnKey === "jenis_barang") {
        return (
          <span className="capitalize">{getKeyValue(item, columnKey)}</span>
        );
      } else if (columnKey === "deskripsi") {
        return trunscatetext(getKeyValue(item, columnKey), 30);
      } else if (columnKey === "foto_barang") {
        return (
          <img
